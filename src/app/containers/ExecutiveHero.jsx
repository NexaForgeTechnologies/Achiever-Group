...
export default function ExecutiveHero() {
    return (
        <>
            <section
                className="bg-[url('/executive-hero.png')] bg-no-repeat bg-cover bg-center
        w-screen ml-[calc(50%-50vw)] px-5 sm:px-10 md:px-18 lg:px-20 grid grid-cols-1 lg:grid-cols-2 py-10 lg:py-20
        "
            >
                {/* Top intro block (visible only on small screens) */}
                <div className="flex flex-col gap-y-4 items-center lg:hidden">
                ...
                {/* Full content (with intro heading visible only on lg+) */}
                <section className="flex flex-col gap-y-4 pr-0 lg:pr-10 items-center lg:items-start lg:row-start-1">
                    {/* Intro block (hidden on small screens) */}
                    <div className="hidden lg:flex flex-col items-start gap-y-3">
                        <Heading
                            name={"Meet the Founder"}
                            maxWidth="max-w-[190px]"
                        />
                    ...
                    <div className="hidden lg:block my-1 text-start">
